      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 scroll-smooth">
            {messages.map((msg, i) => (
              <div key={i} className={cn("flex gap-4 max-w-[800px]", msg.role === 'user' ? "ml-auto flex-row-reverse" : "")}>
                <div className={cn("w-10 h-10 rounded flex items-center justify-center shrink-0 glass", msg.role === 'assistant' ? "bg-primary/20" : "bg-white/5")}>
                  {msg.role === 'assistant' ? <Bot className="w-6 h-6 text-primary" /> : <User className="w-6 h-6 text-white/40" />}
                </div>
                <div className={cn("p-5 rounded text-sm leading-relaxed", msg.role === 'assistant' ? "bg-white/5 border border-white/5" : "bg-primary text-white")}>
                  {msg.content}
                </div>
              </div>
            ))}
            {loading && (
              <div className="flex gap-4 max-w-[800px]">
                <div className="w-10 h-10 rounded bg-primary/20 flex items-center justify-center glass shrink-0">
                  <Bot className="w-6 h-6 text-primary" />
                </div>
                <div className="p-5 rounded bg-white/5 border border-white/5">
          ...
          <div className="p-8">
            <div className="max-w-[800px] mx-auto space-y-6">
              ...
              <div className="relative">
                <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Pergunte qualquer coisa sobre o projeto..." className="h-16 pl-6 pr-24 bg-white/5 border-white/10 rounded focus-visible:ring-primary focus-visible:ring-offset-0 placeholder:text-white/20" />
                <div className="absolute right-2 top-2 bottom-2 flex gap-2">
                  <Button variant="ghost" size="icon" className="rounded hover:bg-white/5 h-full w-12 text-white/40">
                    <Mic className="w-5 h-5" />
                  </Button>
                  <Button onClick={handleSend} disabled={loading} className="bg-primary hover:bg-primary/90 rounded h-full w-12">
        ...
        <aside className="w-80 border-l border-white/10 p-8 hidden xl:block bg-black/20">
          ...
          <div className="space-y-4">
            <Card className="glass border-white/5 p-4 hover:bg-white/5 transition-colors cursor-pointer rounded">

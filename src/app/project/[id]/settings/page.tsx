...
export default function SettingsPage() {
  ...
  if (loading) return <div className="p-8 space-y-4"><div className="h-8 w-48 rounded shimmer" /><div className="h-64 rounded-2xl shimmer" /></div>;
    ...
    >
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => router.back()}
            className="p-2 rounded-xl glass hover:bg-white/10 transition-all text-white/40 hover:text-white"
          >
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-black transition-all hover:opacity-90 disabled:opacity-50"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Identidade Visual */}
        <section className="glass rounded-3xl p-6 border-white/5 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-brand outline-none transition-all resize-none"
              />
            </div>
          </div>
        </section>
        ...
        {/* Danger Zone */}
        <section className="md:col-span-2 rounded-3xl p-6 border border-red-500/20 bg-red-500/5 space-y-6">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {!showDeleteConfirm ? (
              <button
                ...
                className="px-6 py-2.5 rounded-xl border border-red-500/30 text-red-500 text-xs font-bold hover:bg-red-500 hover:text-white transition-all"
              >
                Excluir Projeto
              </button>
            ) : (
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-white/5 text-xs font-bold"
                >
                  Cancelar
                </button>
                <button
                  onClick={handleDelete}
                  className="px-6 py-2.5 rounded-xl bg-red-500 text-white text-xs font-bold hover:bg-red-600 transition-all shadow-lg shadow-red-500/20"
                >
                  Confirmar Exclusão
                </button>
              </div>

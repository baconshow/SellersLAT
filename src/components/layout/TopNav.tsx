...
export default function TopNav({
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none" style={{ height: 100 }}>
        <div className="absolute inset-0" style={{
          background: `linear-gradient(to bottom, rgba(5,5,8,0.97) 0%, rgba(5,5,8,0.82) 45%, rgba(5,5,8,0.20) 78%, transparent 100%)`
        }} />
        <div className="absolute inset-0" style={{
          background: `linear-gradient(to bottom, ${accent}12 0%, transparent 60%)`
        }} />

        <div className="relative z-10 flex items-center justify-between px-8 pointer-events-auto" style={{ height: 64 }}>
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              <Link href="/dashboard" className="text-xs font-semibold tracking-widest transition-colors"
              ...
              {!isDashboard && projectId && (
                <>
                  <ChevronRight style={{ width: 11, height: 11, color: 'rgba(255,255,255,0.15)' }} strokeWidth={2.5} />
                  <Link href={`/project/${projectId}`} className="text-xs font-semibold tracking-widest transition-colors"
            ...
            <nav className="flex items-center gap-0.5">
              {navItems.map((item) => {
                ...
                if ((item as any).isLAT) {
                  return (
                    <button key="lat" onClick={onToggleLAT} className="group relative flex items-center justify-center w-10 h-10 rounded-md transition-all">
                      <Icon style={{
                        width: 20, height: 20,
                      ...
                      <span className="absolute top-full mt-2 px-2.5 py-1.5 rounded-md text-[10px] font-medium whitespace-nowrap pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50"
                        style={{ background: 'rgba(14,14,20,0.96)', border: '1px solid rgba(255,255,255,0.07)', color: '#F0F0F5' }}>
                        LAT Intelligence
                      </span>
                    </button>
                  )
                ...
                return (
                  <Link key={item.href} href={item.href} className="group relative flex items-center justify-center w-10 h-10 rounded-md transition-all">
                    <Icon style={{
                      width: 20, height: 20,
          ...
          <div className="flex items-center gap-2 shrink-0">
            <ThemeToggle />
            {!isDashboard && project && (
              <button onClick={() => setDrawerOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-md text-xs font-semibold tracking-wide transition-all"
                style={{ background: `${accent}15`, border: `1px solid ${accent}30`, color: accent }}
                onMouseEnter={e => { e.currentTarget.style.background = `${accent}25`; e.currentTarget.style.borderColor = `${accent}55` }}
            ...
            {isDashboard && onNewProject && (
              <button onClick={onNewProject} className="flex items-center gap-2 px-4 py-2 rounded-md text-xs font-semibold tracking-wide transition-all"
                style={{ background: `${accent}15`, border: `1px solid ${accent}30`, color: accent }}>
                <Plus style={{ width: 13, height: 13 }} strokeWidth={2.5} />
                Novo Projeto
              </button>
      ...
      <AnimatePresence>
        {drawerOpen && project && (
          <WeeklyUpdateDrawer project={project} onClose={() => setDrawerOpen(false)} />
        )}
      </AnimatePresence>
    </>
  )
